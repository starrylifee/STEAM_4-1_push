...
class AebiAdventure extends BaseGame {
  constructor() {
    ...
    this.briefing = {
      ...
      lines: [
        "잘한 점: 행성 7개와 보스·깃발로 모험의 목표가 분명해졌어요. '맞히면 보상' 의견도 코인으로 반영했어요.",
        "짚을 점: 도형 이동 문제가 '보기에서 고르기'라, 실제로 도형을 밀고·돌리고·뒤집어 보는 조작은 아니에요. '밀기'의 본질이 게임 동작에는 아직 덜 들어갔어요.",
        "[AI 코멘트] 다음엔 직접 도형을 움직여서 맞히는 방식이면 수학과 더 딱 붙을 거예요.",
      ],
    };
    this.reset();
  }
...
class ObstacleDodge extends BaseGame {
  constructor() {
    ...
    this.briefing = {
      ...
      lines: [
        "잘한 점: 규칙을 번호로 꼼꼼히 정리했어요(실패 문구·코인·성별·차 구매까지). 20레벨·30코인도 잘 설계했어요.",
        "짚을 점: 이 게임의 '밀기'는 손가락으로 차를 미는 거라 재미있지만, 수학에서 배우는 '평면도형의 밀기(이동)'와는 다른 밀기예요. 도형 이동 개념은 사실 이 게임에는 안 들어가 있어요.",
        "[AI 코멘트] 5개 중 수학 연결이 가장 약한 게임이에요. 재미는 충분하니, 도형을 미는 요소를 더하면 더 좋겠어요.",
      ],
    };
    this.reset();
  }
...
class TargetShooter extends BaseGame {
  constructor() {
    ...
    this.briefing = {
      ...
      lines: [
        "잘한 점: 포차(트럭 대포) 그림이 구체적이라 그대로 새로 그렸고, 명중하면 아이템을 받게 했어요.",
        "짚을 점: 도형 문제는 여기서도 '보기 고르기' 퀴즈라, 도형을 직접 돌리거나 뒤집는 조작까지는 아니에요. '이동'의 본질은 절반만 들어간 셈이에요.",
        "[AI 코멘트] 그래도 문제→대포 성능 상승 흐름이 공부할 이유를 만들어 줘서 도형 문제는 유지했어요.",
      ],
    };
    this.reset();
  }
...
class HouseBuilder extends BaseGame {
  constructor() {
    ...
    this.briefing = {
      ...
      lines: [
        "잘한 점: 단계마다 5포인트, 번 포인트로 집 꾸미기 — 보상 설계를 깊이 고민했어요.",
        "짚을 점: 확률 뽑기 상자(10%·30%…)는 사실상 '도박'에 가까워요. 운에 기대는 보상은 공정하지 않고 도형 공부와도 관계가 없어서, 노력한 만큼 받는 보상으로 바꿨어요.",
        "[AI 코멘트] 대신 칭찬! 재료를 270도까지 직접 '돌리기' 해서 맞추니, 5개 중 '돌리기(이동)'의 본질이 가장 진짜로 들어간 게임이에요.",
      ],
    };
    this.reset();
  }
...
class BulletDefense extends BaseGame {
  constructor() {
    ...
    this.briefing = {
      ...
      lines: [
        "잘한 점: 원래 규칙을 스스로 고쳐 '직접 피하기'로 바꾼 디버깅이 훌륭했어요. 이름(피해라)과도 딱 맞아요.",
        "짚을 점: 대신 원래 있던 '평면도형의 이동' 학습 요소가 빠졌어요. 그래서 이 게임만으로는 수학 공부가 되지 않아요.",
        "[AI 코멘트] 재미는 늘었으니, 수학 목표는 에이비·집만들기 같은 다른 게임이 채우도록 했어요.",
      ],
    };
    this.reset();
  }
